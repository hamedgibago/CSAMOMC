...
# Edited for scheduling workflows by Reza Akraminejad
...
def initial_population(population_size = 5, min_values = [-5,-5], max_values = [5,5], target_function = target_function):
	...
	for i in range(0, population_size):
		...
		population[i,-1] = target_function(population[i, 0:population.shape[1]-1],0,i)
	return population
	...
	for i in range(0, population.shape[0]):
		...
		for j in range(0, len(min_values)):
			if (rand >= ap):
				...
				population[i,j] = random.uniform(min_values[j], max_values[j]) 
		population[i,-1] = target_function(population[i, 0:population.shape[1]-1],iterNum,i)
	return population
