...
class heftCore:
    ...
    def wbar(self,ni, agents, compcost):
        """ Average computation cost """
        #import math
        #avg= sum(compcost(ni, agent) for agent in agents) / len(agents)
        #avg=round(avg)
        #print('avg {} = {}'.format(ni,avg))
        #return avg
        return sum(compcost(ni, agent) for agent in agents) / len(agents)
    
    def cbar(self,ni, nj, agents, commcost):
    ...
    def ranku(self,ni, agents, succ,  compcost, commcost):
        """ Rank of job
    ...
    def start_time(self,job, orders, jobson, prec, commcost, compcost, agent):
    ...
    def recvs(self,job, jobson, prec, recv):
    ...
    def sends(self,job, jobson, succ, send):
    ...
    eps = 1e-9
    def insert_recvs(self,order, jobson, prec, recv):
        if not order:
            return order
    ...
    def insert_sends(self,order, jobson, succ, send):
        if not order:
            return order
    ...
    def insert_sendrecvs(self,orders, jobson, succ, send, recv):
